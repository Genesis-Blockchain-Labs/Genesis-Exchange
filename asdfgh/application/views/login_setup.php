							...
							<div class="ref_admin">
								<div class="radio-col">
									<?php  $attributes = array('role'=>"form", 'method'=>'post');
											$url = base_url().'login_setup_update';
											echo form_open_multipart($url, $attributes); ?>
									<label>Login Functionality: </label> 
									<input type="radio" name="login_status" value="1" <?php if($admin_setup['login_status']==1) { ?>checked="checked" <?php } ?>/><span> ON</span> 
									<input type="radio" name="login_status" value="0" <?php if($admin_setup['login_status']==0) { ?> checked="checked" <?php } ?>/><span> OFF</span> 
